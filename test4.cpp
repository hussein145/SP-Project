...
Menu menu;
RenderWindow window(VideoMode(1920, 1080), "icyTower", Style::Close | Style::Default);
int GameMode;
...
void Gameplay()
{
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(Play))
		{
			...
			//cout << "hussein" << endl;
			if (Play.type == Event::KeyReleased) {
				if (Play.key.code == Keyboard::Escape)  //&& !menu.pressed
				{
					RenderWindow pause(VideoMode(800, 800), "Pause menu", Style::Close | Style::Default);
					PauseMenu pausemenu(300, 300);
					while (pause.isOpen()) {
						sf::Event event;
